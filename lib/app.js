...
const app = net.createServer(socket => {
    socket.on('data', data => {
        ...
        if(request.path === '/red' && request.method === 'GET'){
            return socket.end(createResponse({ body: '<h1>red</h1>', status: '200', contentType: 'text/html' }));
        }
        if(request.path === '/green' && request.method === 'GET'){
            return socket.end(createResponse({ body: '<h1>green</h1>', status: '200', contentType: 'text/html' }));
        }
        if(request.path === '/blue'){
            return socket.end(createResponse({ body: '<h1>blue</h1>', status: '200', contentType: 'text/html' }));
        }
